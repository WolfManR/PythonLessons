...
clearedText = text
for mark in marks:
    clearedText = clearedText.replace(mark, '')
...
words = ['я', 'ты', 'мы', 'они', 'вы']
listOfWords = list(filter(lambda word: len(word) > 3 or word in words, clearedText.split(' ')))
print(listOfWords)

# 3     привести все слова к нижнему регистру (map);
listOfLowerWords = list(map(lambda x: x.lower(), listOfWords))
print(listOfLowerWords)

# 4     получить из list пункта 3 dict, ключами которого являются слова, а значениями их количество появлений в тексте;
wordsCounter = {}
for x in listOfLowerWords:
    wordsCounter[x] = 1 if x not in wordsCounter.keys() else wordsCounter[x] + 1
print(wordsCounter)

# 5     вывести 5 наиболее часто встречающихся слов (sort), вывести количество разных слов в тексте (set).
sortedWordsCounter = sorted(wordsCounter.items(), key=lambda item: item[1], reverse=True)
print(f'5 часто встречающихся слов в тексте: {sortedWordsCounter[:5]}')

print(f'число уникальных слов в тексте: {len(set(listOfLowerWords))}')
